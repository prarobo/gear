...
class MessageMonitor(object):
    ...
    def _loadParameters(self):
        ...
        rospy.loginfo("[DiskMonitor] update interval : "+self.update_interval)
        rospy.loginfo("[DiskMonitor] low free space : "+self.low_free_space)
        rospy.loginfo("[DiskMonitor] critical free space : "+self.critical_free_space)
        return
    ...
    def _diskCheckTask(self, stat):
        ...
        if self._getAvailableDiskSpace() < self.critical_free_space:
            stat.summary(diagnostic_msgs.msg.DiagnosticStatus.ERROR, "Disk space CRITICAL")
        elif self._getAvailableDiskSpace() < self.low_free_space:
            stat.summary(diagnostic_msgs.msg.DiagnosticStatus.WARN, "Disk space LOW")
        else:
            stat.summary(diagnostic_msgs.msg.DiagnosticStatus.OK, "Disk space OK")
        return stat
